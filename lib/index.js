...
var SpawnMocha = function (opts) {
  ...
  var queue = async.queue(function (task, done) {
    ...
    let transform = through(function write(line) {
      this.queue((overrideOpts.liveOutputPrepend || "") + line + "\n")
    })

    let file = fs.createWriteStream('/tmp/mocha.'+task.taskNum, {encoding: 'utf8'});
    file.on('open', function() {
      let child = proc.spawn(bin, args.concat(task.files), {env: env});
      if(overrideOpts.liveOutput) {
        child.stdout.pipe(split()).pipe(transform).pipe(file)
      } else {
